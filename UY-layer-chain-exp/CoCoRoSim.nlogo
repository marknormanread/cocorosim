...
to startup ; #################################################################################################################################
  ...
  load-shapes-3d "./core/Jeff.shape"
...
to setup ; #################################################################################################################################
  ...
  ;Set start coordinates of camera for panning
  ;set ypos -25
  ;set zpos 5
  ;set zoom-val -10
  
  ;; set up the camera  
  ;setxyz 0 ypos zpos ; sets the position of camera ; user
  ;face patch 0 0 0  ; sets the heading of camera ; user
  ;zoom zoom-val ; sets the zoom of camera ; user
  
  ;let auv-list [ who ] of all-AUVs 
  ;foreach auv-list
  ;[ let the-next-id [chain-next-id] of turtle ?
  ;print (word "turtle " ? " has next auv " the-next-id) 
  ;]

  ;ask patch 0 0 20 [ set pcolor green ]
...
to go ; #################################################################################################################################
  ...
  ;;Changed this so it isn't hardcoded
  set time time + ticklength ; increase the real time by the right amount; at the moment one tick is 0.5 seconds.
...
to record-movie
  ...
  ;tick 0.2 used for videos, to make them look nicer
  set ticklength 0.2
     
  movie-start "chain-formation-vid-test.mov"
  ...
  let finalzoom -70
  ;Set camera panning
  let pan-duration 100
  let start-pan 300
  
  ;Set duration in seconds
  let movie-duration 90
  
  ;ask AUVs-Lily with [who = 0] [ chain-seed-chain ]
  repeat 30 * movie-duration
  [
    ;if ticks = 1 
    ;[ 
     ; ask one-of AUVs-Lily with [chain-alg-state = "chain" and chain-end-link?]
     ; [ chain-build-recruit-next ]
    ;]
    
    if  (ticks > start-pan) and (ticks <= start-pan + pan-duration)
    [
      ;;Move the camera incrementally to give the impression of panning
      set ypos (ypos + ((finalypos - ypos) / pan-duration))
      set zpos (zpos + ((finalzpos - zpos) / pan-duration))
      set zoom-val (zoom-val + ((finalzoom - zoom-val) / pan-duration))
...
247
recruit-start
ask one-of AUVs-Lily with [chain-alg-state = \"chain\" and chain-start-link?]\n[\n  chain-build-recruit-prev\n]
NIL
1
T
OBSERVER
...
839
56
A randomised component added to gps to reduce accuracy (in cm)\n
11
0.0
1
